...
Allocating memory:
  ...
  If not we just increase the heap adn create a new free block.
...
/*Function return a block with minimum size of size from corrext bucket */
static inline word_t *find_free_blk(size_t size){
  word_t *res = free_list;
  int var = 2;
  size_t blocksize = size >> 4;
  
  while(blocksize >= var && var <= MAX_BUCKET){ //Find the lowest bucket in which we could hold our free block
    res += 4;
    var = var << 1;
  }
  ...
  while(search != res){ //Check if there is a block ot the expected size
    if(bt_size(search) >= size)
      return search;
    search = bt_get_nextptr(search);
  }
  
  if(var <= MAX_BUCKET) //If res is not pointing to the last bucket we try to find it in a next one, 
    res += 4;           //in which we will be sure that all blocks are greater/equal then our requested 
                        //size if the bucket is not empty
  while(bucket_empty(res) && var < MAX_BUCKET){
    res += 4;
    var = var << 1;
  }
...
/*Function calculate in which bucket we want to insert our free block */
static inline word_t *which_bucket(size_t size){ 
  word_t *res = free_list;
  size = size >> 4;
  int var = 2;
  while(size >= var && var <= MAX_BUCKET){
    res += 4;
    var = var << 1;
  }
  return res;
...
int mm_init(void) {
  ...
  int x = 19;  //init rest of free block lists
  while(x--){
...
void mm_checkheap(int verbose) {
  ...
  int x = 20;
  while(x--){
    word_t *search = bt_get_nextptr(list); //check the free list, if each element is free, size and addres are correct

...
static noreturn void coro_switch(int v) {
  ...
  //safe_printf("[==] %s :: %d :: %d\n", running->co_name, v, TAILQ_EMPTY(&runqueue));

  /*
  coro_t *it;
  TAILQ_FOREACH(it, &runqueue, co_link) {
    safe_printf("[====] %s\n", it->co_name);
  }
  ...
  */
  ...
  coro_t *co = running;
  running = TAILQ_NEXT(co, co_link);
  if (running == NULL) {
    running = TAILQ_FIRST(&runqueue);
  }

  if (v == EOF) {
    TAILQ_REMOVE(&runqueue, co, co_link);
  }

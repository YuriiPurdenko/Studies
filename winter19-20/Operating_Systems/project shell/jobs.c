    ...
    while((pid = waitpid(-1, &status, WNOHANG|WUNTRACED|WCONTINUED)) > 0){
      for(int g = 0; g < njobmax; g++){
        ...
        for(int i = 0; i < jobs[g].nproc; i++){
          ...
          if(jobs[g].proc[i].state != FINISHED)
            jobstatus = jobs[g].proc[i].state;
        }
        jobs[g].state = jobstatus;
      }
    }
  errno = old_errno;
}
...
bool resumejob(int j, int bg, sigset_t *mask) {
  ...
  if(!bg){
    movejob(j, FG);
    int var;
    while(jobstate(FG, &var) == STOPPED){   //czekanie, az do wszystkich podprocesow dojdzie sygnal (wyscig :( )
...
bool killjob(int j) {
  ...
  if(jobs[j].state == STOPPED)  //umozliwienie dotarcia sygnalu do zastopowanych procesow
    Kill(-jobs[j].pgid, SIGCONT);
  return true;
}
...
int monitorjob(sigset_t *mask) {
  ...
  Kill(-jobs[FG].pgid, SIGCONT);
  //RACE (SIGSTOP DELIVERED = ENDLESS LOOP)
  ...
  if(state == STOPPED){
    int j = allocjob();
    jobs[j].pgid = 0;   //aby spelnic asserty move

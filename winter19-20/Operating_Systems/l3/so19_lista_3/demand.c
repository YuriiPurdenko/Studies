...
static void sigsegv_handler(int signum, siginfo_t *info, void *data) {
  ucontext_t *uc = data;
  unsigned long int addr = (unsigned long int)info->si_addr;
  unsigned long int page = addr - (addr % pagesize);
  unsigned long int pc = uc->uc_mcontext.gregs[REG_RIP];

  /* TODO: Something is missing here! */
  if (addr < (unsigned long int)VA_START || addr >= (unsigned long int)VA_END) {
    safe_printf(
        "Fault at 0x%lx accessing 0x%lx! Address not mapped - terminating!\n",
        pc, addr);
    _exit(info->si_signo);
  }
  ...
  case SEGV_MAPERR: {
    ...
    safe_printf("Fault at 0x%lx accessing 0x%lx! Map missing page at 0x%lx\n",
                pc, addr, page);
    break;
  }
  case SEGV_ACCERR: {
    mprotect_page((void*)page, PROT_READ | PROT_WRITE);
    safe_printf("Fault at 0x%lx accessing 0x%lx! Make page at 0x%lx writable\n",
                pc, addr, page);
    break;
  }
  default:
    safe_printf(
        "Fault at 0x%lx accessing 0x%lx! Usuported errno [%d] exit with [%d]\n",
        pc, addr, info->si_code, info->si_signo);
    _exit(info->si_signo);
  }
}

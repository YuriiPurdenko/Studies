 ...
 */
#define MM_BUCKETS 10
static block_t *mm_list[MM_BUCKETS];
static int32_t mm_size[MM_BUCKETS] = {
  48,
  96,
  192,
  384,
...
static inline int get_bucket_idx(int32_t size) {
  for (int i = 0; i < MM_BUCKETS; ++i)
    if (mm_size[i] >= size)
      return i;
  return MM_BUCKETS;
...
static void mm_list_insert(block_t *block, block_t **head) {
  assert(block_is_free(block));
  ...
  block->payload.free.prev = NULL;
  block->payload.free.next = *head;
  (*head)->payload.free.prev = block;
  *head = block;
}
...
static void split(block_t *block, int32_t size) {
  ...
  debug("split %p (0x%x) into %p (0x%x) %p (0x%x)\n", block, old_size, block,
        size, new_block, old_size - size);
...
static void mm_list_erase(block_t *block, block_t **head) {
  if (*head == block) {
    *head = block_free_next(block);
  }

  /* A <-> block <-> B
   * A -> B
   */
  if (block_free_prev(block))
...
void *malloc(size_t size) {
  ...
  debug("malloc -> idx %d\n", idx);
  block_t *block = bf(mm_list[idx], size);

  if (NULL == block) {
    for (int i = idx + 1; i < MM_BUCKETS; ++i) {
      block_t *block = bf(mm_list[i], size);
      if (block) {
        idx = i;
        break;
      }
  ...
  if (next) {
    mm_list_erase(next, &mm_list[idx]);
    int32_t nsize = block_size(next);
    int nidx = get_bucket_idx(nsize);
    debug("malloc -> insert %p (0x%x) %d\n", next, nsize, nidx);
    mm_list_insert(next, &mm_list[nidx]);
  }

  mm_list_erase(block, &mm_list[idx]);
  block_set_used(block);
...
void merge(block_t *block, block_t *next_block) {
  ...
  int b_idx = get_bucket_idx(b_size);
  int next_b_idx = get_bucket_idx(next_b_size);
  int idx = get_bucket_idx(size);

  mm_list_erase(block, &mm_list[b_idx]);
  mm_list_erase(next_block, &mm_list[next_b_idx]);

  block_set_size(block, size);

  debug("merge -> %p (0x%x) %p (0x%x) -> %d\n", block, b_size, next_block,
        next_b_size, idx);

  mm_list_insert(block, &mm_list[idx]);
}
...
void mm_checkheap(int verbose) {
  for (int i = 0; i < MM_BUCKETS; ++i) {
    block_t *head = mm_list[i];
    for (block_t *block = head; NULL != block;
         block = block->payload.free.next) {
      assert(block_is_free(block));
      assert(block_prev_is_used(block));
      assert((int64_t)block_payload(block) % ALIGNMENT == 0);
      assert(get_bucket_idx(block_size(block)) == i);
    }
  }
  return;
}

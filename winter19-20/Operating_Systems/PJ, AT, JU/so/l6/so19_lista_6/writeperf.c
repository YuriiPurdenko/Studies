...
int main(int argc, char *argv[]) {
  ...
  if (strncmp(choice, "fwrite", 6) == 0) {
    ...
    buf = malloc(size);
    if (setvbuf(stdout, buf, mode, size)) {
      unix_error("setvbuf error\n");
    }
  ...
  if (strcmp(choice, "writev") == 0) {
    ...
    for (int j = 0; j < times; ++j) {
      for (int k = 0; k < length; ++k) {
        iov[k % n].iov_base = line + k;
        iov[k % n].iov_len = length + 1 - k;
        if (k % n == n - 1) {
          writev(STDOUT_FILENO, iov, n);
        }
      }
      if (length < n || length % n != 0) {
        writev(STDOUT_FILENO, iov, length < n ? length : length % n);
      }
    }
  }

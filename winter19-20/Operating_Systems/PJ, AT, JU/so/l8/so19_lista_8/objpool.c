...
static arena_t *init_arena(arena_t *ar) {
  ...
  // for every object in arena we need extra bit in bitmap
  size_t nitems = 8 * freemem / (8 * sizeof(object_t) + 1);
  ar->nitems = nitems;
  ar->nfree = nitems;
...
static void *alloc_block(arena_t *ar) {
  ...
  bit_ffc(ar->bitmap, ar->nitems, &index); // find number of first not set bit

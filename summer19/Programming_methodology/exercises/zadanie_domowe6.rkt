...
(define (heap-merge h1 h2)
  (cond
   [(leaf? h1) h2]
   [(leaf? h2) h1]
   [else (if (<= (elem-priority (hnode-elem h1)) (elem-priority (hnode-elem h2)))
...
(define make-hnode-test
  (test-suite
   ...
   (test-case
    "Type match"
    (hnode? (make-hnode (cons 1 2) leaf leaf)))
   (test-case
    "Type match"
    (hnode? (make-hnode (cons 6 5) leaf leaf)))
   (test-case
    "Type match"
    (hnode? (make-hnode (cons 2 2) (make-hnode (cons 3 2) leaf leaf) leaf)))
   (test-case
    "Type match"
    (hnode? (make-hnode (cons 1 1) leaf (make-hnode (cons 4 4) leaf leaf))))
   (test-case
    "Type match"
    (hnode? (make-hnode (cons 1 1) (make-hnode (cons 2 3) leaf leaf) (make-hnode (cons 5 4) leaf leaf))))))
